...
class _MaintenanceDetailScreenState extends State<MaintenanceDetailScreen> {
  ...
  Future<({bool ok, double? lat, double? lng})> _verifyOnSite(
      Maintenance m, String action) {
    return positionVerifiee(context,
        siteLat: m.siteLatitude,
        siteLng: m.siteLongitude,
        siteNom: m.siteNom,
        action: action);
  }
...
class _CloseSheetState extends State<_CloseSheet> {
      ...
      return Expanded(
        child: OutlinedButton.icon(
          onPressed: () => setState(() {
            ...
            // (sinon absent + signature d'agent = contradiction).
            if (!value) {
              _nomAgent.clear();
              _sigAgent = null;
            }
  ...
  Future<void> _capturerSignatureAgent() async {
    ...
    final path = await AttachmentStore.persistBytes(
        bytes as Uint8List, 'signature-agent-${widget.maintenance.id}.png');
