...
class _MaintenanceFormScreenState extends State<MaintenanceFormScreen> {
  ...
  Future<void> _loadActifs() async {
    setState(() { _loadingActifs = true; _actifs = []; _actif = null; });
  ...
  Future<void> _pickDate() async {
    ...
    final d = await showDatePicker(context: context, initialDate: init, firstDate: today, lastDate: now.add(const Duration(days: 365)));
  ...
  Future<void> _submit() async {
    ...
    String? payloadSiteId, type, categorie, equipement, tacheKey, actifType, actifId, siteSourceId;
    if (_isEntretien) {
      ...
      if (_needsDest && _siteId == null) { messenger.showSnackBar(const SnackBar(content: Text('Sélectionnez le site de destination'))); return; }
      payloadSiteId = _nature == 'DESINSTALLATION' ? actif.siteId : _siteId;
      if (payloadSiteId == null) { messenger.showSnackBar(const SnackBar(content: Text('Site indéterminé pour cet actif'))); return; }
      type = 'CURATIVE'; categorie = actif.categorie;
      equipement = '${kNatureTravaux[_nature]} — ${actif.libelle ?? actif.categorie}';
      actifType = actif.actifType; actifId = actif.id;
    ...
    try {
      ...
      if (!mounted) return;
      messenger.showSnackBar(SnackBar(content: Text(res.isQueued ? 'Hors-ligne : enregistré et mis en file de synchronisation' : 'Intervention planifiée')));
      router.pop();
    } catch (e) {
        ...
        child: ListView(
          ...
          children: [
            DropdownButtonFormField<String>(
              ...
              isExpanded: true,
              decoration: const InputDecoration(labelText: 'Nature des travaux *'),
              items: _natureOptions.map((o) => DropdownMenuItem(value: o.$1, child: Text(o.$2, overflow: TextOverflow.ellipsis))).toList(),
            ...
            if (_isEntretien) ...[
              ...
              DropdownButtonFormField<String>(
                ...
                decoration: InputDecoration(
                  ...
                  suffixIcon: _loadingTaches ? const Padding(padding: EdgeInsets.all(12), child: SizedBox(height: 16, width: 16, child: CircularProgressIndicator(strokeWidth: 2))) : null,
                ),
              ...
              DropdownButtonFormField<String>(
                ...
                isExpanded: true,
                decoration: InputDecoration(
                  labelText: _nature == 'INSTALLATION' ? 'Actif au dépôt *' : 'Actif en service *',
                  suffixIcon: _loadingActifs ? const Padding(padding: EdgeInsets.all(12), child: SizedBox(height: 16, width: 16, child: CircularProgressIndicator(strokeWidth: 2))) : null,
                ),
                items: _actifs.map((a) => DropdownMenuItem(value: a.id, child: Text(a.display, overflow: TextOverflow.ellipsis))).toList(),
                onChanged: (v) => setState(() => _actif = _actifs.where((a) => a.id == v).firstOrNull),
                validator: (v) => !_isEntretien && (v == null || v.isEmpty) ? 'Requis' : null,
              ),
              if (_needsDest) ...[
                const SizedBox(height: 14),
