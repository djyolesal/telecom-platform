...
async function resolvePrestataireId(siteId: string, categorie: string): Promise<string | null> {
  ...
  const scope = PASSIVE_CATS.includes(categorie)
    ...
    : ACTIVE_CATS.includes(categorie)
      ? 'ACTIVE'
      : null;
  // Périmètre spécifique d'abord, puis "les deux". Le tri `scope: 'asc'` suit
  // l'ordre de DÉCLARATION de l'enum Postgres (PASSIVE < ACTIVE < LES_DEUX),
  // pas l'ordre alphabétique : le scope spécifique précède donc toujours
  // LES_DEUX. Même idiome que planning.service et slaCompliance.service.
