 ...
 * 0 = désactivée) : récapitule les incidents terrain encore ouverts qui
 * DÉPASSENT le seuil (1 h par défaut) — côté passif uniquement (les coupures
 * partielles ont été retirées du récap à la demande de l'exploitant). Même
 * aiguillage que les notifications unitaires : chaque contact de prestataire
 * ne reçoit QUE son périmètre (sites de ses lots), les contacts « toutes
 * sociétés » reçoivent tout. Rien à signaler → aucun SMS. Un contact = un SMS
 * maximum par situation.
 */
export async function situationPeriodiqueJob(): Promise<void> {
  ...
  const composer = (incs: { nom: string; h: number }[]) =>
    incs.length ? `[E&M OpS] Situation ${heure} - ${incs.length} incident(s) >${seuilH}h : ${enListe(incs)}` : null;
  ...
  for (const c of contacts) {
    const message = c.toutesSocietes
      ? composer(incGlobal)
      : c.prestataireId
        ? composer(incParPresta.get(c.prestataireId) ?? [])
  ...
  logger.info(`[situation] ${incidents.length} incident(s) → ${[...parMessage.values()].flat().length} contact(s)`);
}
